...
{isModalOpen && (
  <div className="fixed inset-0 bg-gray-900 bg-opacity-60 flex justify-center items-center z-20 transition-opacity duration-300 opacity-92">
    ...
    <span className="text-red-500 font-semibold">(🚫 Reported and disabled🙅 Removed by admin .)</span>
              ...
              <p className="text-gray-700 mt-2" dangerouslySetInnerHTML={{ __html: expandedPosts.has(post._id) ? post.description : post.description.length > 150 ? `${post.description.slice(0, 150)}...` : post.description }}></p>
